...
public class EmployeeManagement {
    ...
    private static final Supplier<Stream<String>> supplier = () -> {
        try {
            FileReader fr = new FileReader("./resources/streamingvf.csv");
    ...
    private static String firstLine() {
        Stream<String> lines=supplier.get();
    ...
    private static List<String> filteredLastnames() {
        ...
        Predicate<String> containsOorK = s -> s.contains("O") || s.contains("K");

        Stream<String> lines=supplier.get();
        return lines.filter(predicate.and(containsOorK)).collect(Collectors.toList());
    }
    ...
    private static List<Integer> occurencesOfE() {
        BiFunction<Employee, Character, Integer> countOccurrences = (emp, ch) -> {
            long count = emp.getFirstname().chars()
                    .filter(chInt -> chInt == ch)
                    .count();
            return (int) count;
        };

        return supplier.get().map(Employee::new).map(s->countOccurrences.apply(s,'e')).toList();
    }
    ...
    private static boolean allEmailCorrect() {
        return supplier.get().map(Employee::new).map(Employee::getEmail).allMatch(s -> s.endsWith("streamingvf.be"));
    }
    ...
    private static String longLastName() {
        return supplier.get().map(Employee::new).map(Employee::getLastname).filter(s->s.length()>14).findFirst().orElse("None");
    }

    /**
     * Calcule le nombre d'employé à mi-temps de la boîte
     * @return le nombre d'employé à mi-temps de l boîte
     */
    private static long numbreOfPartTimers() {
        return supplier.get().map(Employee::new).map(e -> !e.isFullTime()).count();
    }
